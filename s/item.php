  ...
  <div class="site-img-header"><img class="logoimg" src="public/img/logo-text.png"></div>
    ...
    <div class="content">
      <?php
        $connection = new mysqli('localhost', 'root', 'Sp00ked', 'frosty');
        $id = $_GET['id'];
        $sql = "SELECT * FROM products WHERE id='$id'"; // Get the details of the item from the products table
        $result = $connection->query($sql);
        while ($row = $result->fetch_array()) { // Display the items details
      ...
      <form action='' method='post' class='item '>
        <?php
          $connection = new mysqli('localhost', 'root', 'Sp00ked', 'frosty');
          $sql = "SELECT stock FROM products WHERE id='$id'"; // Get the stock of the current item being viewed
          $result = $connection->query($sql);
          $row = $result->fetch_array();
          $stock = $row['stock'];
          $user_id = $_SESSION['user'];
          $id = $_GET['id'];
          $sql = "SELECT ammount FROM cart WHERE user='$user_id' AND item_id='$id'"; // Get the ammount currently stored in the users cart of the current item
          $result = $connection->query($sql);
          $row = $result->fetch_array();
          $ammount = $row['ammount'];
          if (($stock - $ammount) > 0) { // Greys the add button if the ammount is equal to stock
          ...
          if ($ammount > -1) { 
            ...
            ";
          } else { // If the ammount is equal to or less than 0 then just display 0
          ...
          ";
          if ($stock > -1) { // If the stock of a item is less than or equal to 0 then show Out of Stock instead of 0
