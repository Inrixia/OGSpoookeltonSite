        ...
        <div class="container-fluid">
          ...
          <div class='purchases_container'>
            ...
            <?php
            	$user_id = $_SESSION['user'];
              $connection = new mysqli('localhost', 'root', 'Sp00ked', 'frosty');
  						$sql = "SELECT * FROM purchases"; // Get every purchase from the purchases table
  						if (!$result = $connection->query($sql)) {}
  						while ($row = $result->fetch_array()) { // For each purchase from purchases display the ID, User, Total, Date and Status
  							$id = $row['purchase_id'];
                $user_id = $row['user_id'];
  							$total = $row['total'];
  							$date = $row['datepur'];
                    ...
                    <th>$user_id</th>
  	    						...
  	    						<th>$$total</th>
                ";
                if ($status == 'Cancelled') { // Color the status appropriately
                  $style = 'red';
                ...
                ";
                if ($status == 'Ordered') { // If the status is ordered then give the ability to Cancel and Ship (Admin only Admin Panel)
                  echo "<th>
