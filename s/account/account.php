      ...
      <div id="wrapper">
        <ul class="nav side-nav ">
          ...
          <?php
            ...
            $sql = "SELECT * FROM users WHERE username='$user_id'"; // Get the user's details from the users table
            $result = $connection->query($sql);
            $row = $result->fetch_array();
            $role = $row['role'];
            if ($role == 'admin') { // If the user is a admin display admin settings
		        	echo "<li>
        ...
        <div class="container-fluid">
          ...
          <div class='purchases_container'>
            ...
            <?php
            	$user_id = $_SESSION['user'];
              $connection = new mysqli('localhost', 'root', 'Sp00ked', 'frosty');
  						$sql = "SELECT * FROM purchases WHERE user_id='$user_id'"; // Get all the purchases for the current user
  						if (!$result = $connection->query($sql)) {}
  						while ($row = $result->fetch_array()) { // Display the id, date, total and status for each order
  							$id = $row['purchase_id'];
  							$total = $row['total'];
  							$date = $row['datepur'];
  							$status = $row['status'];
                  ...
                  <tr>
                    ...
                    <th>$$total</th>
                ";
                if ($status == 'Cancelled') { // Depending on the status colour accordingly
                  $style = 'red';
                ...
                ";
                if ($status == 'Ordered') { // If the purchase isnt cancelled or shipped give the option to cencel (No Ship this isnt admin panel)
                  echo "<th>
